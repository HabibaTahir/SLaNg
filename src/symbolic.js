  ...
  switch (expr.type) {
    ...
    case "var": {
      if (!(expr.name in vars))
        throw new Error(`Variable ${expr.name} not provided`);
      return vars[expr.name];
    }
    case "neg":
      return -symEval(expr.arg, vars);
    case "add":
      return symEval(expr.left, vars) + symEval(expr.right, vars);
    case "mul":
      return symEval(expr.left, vars) * symEval(expr.right, vars);
    case "div":
      return symEval(expr.top, vars) / symEval(expr.bot, vars);
    case "pow":
      return Math.pow(symEval(expr.base, vars), symEval(expr.exp, vars));
    case "fn": {
...
export function symDiff(expr, variable) {
  switch (expr.type) {
    ...
    case "add":
      return symAdd(
        symDiff(expr.left, variable),
        symDiff(expr.right, variable),
      );
    ...
    case "mul": {
      ...
      return symAdd(symMul(du, expr.right), symMul(expr.left, dv));
    }
    ...
    case "pow": {
      ...
      if (expE.type === "const") {
        ...
        return symMul(symMul(symConst(n), symPow(base, symConst(n - 1))), db);
      }
      ...
      return symMul(
        expr,
        symAdd(symMul(dg, ln(base)), symDiv(symMul(expE, df), base)),
      );
    }
...
function _fnDerivative(name, u) {
  switch (name) {
    ...
    case "cos":
      return symNeg(sin(u));
    case "tan":
      return symPow(sec(u), symConst(2));
    case "sec":
      return symMul(sec(u), tan(u));
    case "csc":
      return symNeg(symMul(csc(u), cot(u)));
    case "cot":
      return symNeg(symPow(csc(u), symConst(2)));
    case "asin":
      return symDiv(
        symConst(1),
        sqrt(symSub(symConst(1), symPow(u, symConst(2)))),
      );
    case "acos":
      return symNeg(
        symDiv(symConst(1), sqrt(symSub(symConst(1), symPow(u, symConst(2))))),
      );
    case "atan":
      return symDiv(symConst(1), symAdd(symConst(1), symPow(u, symConst(2))));
    case "sinh":
      return cosh(u);
    case "cosh":
      return sinh(u);
    case "tanh":
      return symSub(symConst(1), symPow(tanh(u), symConst(2)));
    case "ln":
      return symDiv(symConst(1), u);
    case "log10":
      return symDiv(symConst(1), symMul(ln(symConst(10)), u));
    case "exp":
      return exp(u);
    case "sqrt":
      return symDiv(symConst(1), symMul(symConst(2), sqrt(u)));
...
export function symIntegrate(expr, variable) {
  ...
  if (
    ...
    return symDiv(symPow(symVar(variable), symConst(n + 1)), symConst(n + 1));
  }
    ...
    if (d.type === "add") {
      const isOnePlusX2 =
        (d.left.type === "const" &&
          d.left.value === 1 &&
          d.right.type === "pow" &&
          _isVar(d.right.base, variable) &&
          d.right.exp.value === 2) ||
        (d.right.type === "const" &&
          d.right.value === 1 &&
          d.left.type === "pow" &&
          _isVar(d.left.base, variable) &&
          d.left.exp.value === 2);
...
export function symSimplify(expr) {
  ...
  switch (expr.type) {
    ...
    case "div": {
      ...
      if (t.type === "const" && b.type === "const")
        return symConst(t.value / b.value);
    ...
    case "pow": {
      ...
      if (base.type === "const" && e.type === "const")
        return symConst(Math.pow(base.value, e.value));
...
export function slangToSym(fraction) {
  ...
  const numerator =
    numTerms.reduce((acc, t) => (acc ? symAdd(acc, t) : t), null) ||
    symConst(0);
  ...
  if (typeof fraction.deno === "object" && fraction.deno.terms) {
    ...
    const denominator =
      denTerms.reduce((acc, t) => (acc ? symAdd(acc, t) : t), null) ||
      symConst(1);
...
export function computeLimit(expr, variable, value, maxHopital = 5) {
  ...
  const _eval = (x) => {
    try {
      return symEval(expr, { [variable]: x });
    } catch {
      return NaN;
    }
  };
  ...
  if (isFinite(value)) {
    const v = _eval(value);
    if (isFinite(v))
      return { limit: v, method: "direct substitution", exists: true };
  }
  ...
  if (
    isFinite(left) &&
    isFinite(right) &&
    Math.abs(left - right) < 1e-6 * (1 + Math.abs(left))
  ) {
    return {
      limit: (left + right) / 2,
  ...
  if (expr.type === "div") {
    ...
    for (let i = 0; i < maxHopital; i++) {
      const tVal = symEval(symSimplify(top), {
        [variable]: isFinite(value) ? value : 1e15,
      });
      const bVal = symEval(symSimplify(bot), {
        [variable]: isFinite(value) ? value : 1e15,
      });
      const indeterminate =
        (Math.abs(tVal) < 1e-6 && Math.abs(bVal) < 1e-6) ||
      ...
      const result = symEval(candidate, {
        [variable]: isFinite(value) ? value : 1e15,
      });
      if (isFinite(result)) {
        return {
          limit: result,
          method: `L'Hôpital (${i + 1} application${i > 0 ? "s" : ""})`,
          exists: true,
        };
      }
    }
  ...
  return {
    limit: null,
    method: "left and right limits differ",
    exists: false,
    leftLimit: left,
    rightLimit: right,
  };
 ...
 */
export function symFindCriticalPoints(
  expr,
  variable,
  range = [-10, 10],
  samples = 2000,
...
export function analyzeCurve(expr, variable, range = [-10, 10]) {
  ...
  const allX = [
    -999,
    ...classifications.map((c) => c.x),
    ...inflectionData.points,
    999,
  ].sort((a, b) => a - b);
  ...
  return {
    ...
    inflectionPoints: inflectionData.classifications.map((c) => ({
      x: c.x,
      f: symEval(expr, { [variable]: c.x }),
    })),
...
export function integrationByParts(expr, variable, maxDepth = 3) {
  ...
  return _ibpRecurse(expr, variable, maxDepth, steps)
    ? {
        result: symSimplify(_ibpRecurse(expr, variable, maxDepth, steps)),
        steps,
      }
    : null;
}
...
function _liateRank(e) {
  if (
    e.type === "fn" &&
    ["ln", "log10", "asin", "acos", "atan"].includes(e.name)
  )
    return 0; // L/I
...
function _ibpRecurse(expr, variable, depth, steps) {
  ...
  const [u, dv] =
    _liateRank(left) <= _liateRank(right) ? [left, right] : [right, left];
...
export function fourierSeries(expr, variable, L = Math.PI, N = 10) {
  ...
  for (let n = 1; n <= N; n++) {
    an.push(
      (1 / L) * _int((x) => f(x) * Math.cos((n * Math.PI * x) / L), -L, L),
    );
    bn.push(
      (1 / L) * _int((x) => f(x) * Math.sin((n * Math.PI * x) / L), -L, L),
    );
  }
  ...
  for (let n = 1; n <= N; n++) {
    ...
    const arg =
      L === Math.PI
        ? `${n}${variable}`
        : `\\frac{${n}\\pi ${variable}}{${_roundLatex(L)}}`;
...
function _laplaceRule(e, t, s) {
  ...
  // L{t^n} = n!/s^(n+1)
  if (e.type === "var" && e.name === t)
    return symDiv(symConst(1), symPow(s, symConst(2)));
  if (
    e.type === "pow" &&
    e.base.type === "var" &&
    e.base.name === t &&
    ...
    if (n >= 0 && Number.isInteger(n)) {
      const fac = Array.from({ length: n }, (_, i) => i + 1).reduce(
        (a, b) => a * b,
        1,
      );
...
export function zTransform(seqName, params = {}, freqVar = "z") {
  ...
  switch (seqName) {
    case "impulse": // δ[n] → 1
      ...
      break;
    case "step": // u[n] → z/(z−1)
      expr = symDiv(z, symSub(z, symConst(1)));
      break;
    case "ramp": // n·u[n] → z/(z−1)²
      expr = symDiv(z, symPow(symSub(z, symConst(1)), symConst(2)));
      break;
    case "exponential": {
      // a^n·u[n] → z/(z−a)
      const a = symConst(params.a ?? 1);
      expr = symDiv(z, symSub(z, a));
      break;
    }
    case "sin": {
      // sin(ωn)·u[n] → z·sin(ω)/(z²−2z·cos(ω)+1)
      const w = params.omega ?? 1;
      const sw = symConst(Math.sin(w)),
        cw = symConst(Math.cos(w));
      expr = symDiv(
        symMul(z, sw),
        symAdd(
          symSub(symPow(z, symConst(2)), symMul(symConst(2), symMul(z, cw))),
          symConst(1),
        ),
      );
      break;
...
export function parseExpr(src) {
  ...
  function consume(expected) {
    ...
    if (expected && t !== expected)
      throw new Error(`parseExpr: expected "${expected}" got "${t}"`);
    return t;
  }
  ...
  if (pos < tokens.length)
    throw new Error(
      `parseExpr: unexpected token "${tokens[pos]}" at position ${pos}`,
    );
  return result;
}
...
const _GL_NODES = [
  ...
  0.2392873622521371, 0.3318686022821276, 0.4213512761306353,
...
const _GL_WEIGHTS = [
  0.0070186100094491, 0.0162743947309057, 0.0253920653092621,
  ...
  0.0162743947309057, 0.0070186100094491,
];
...
export function doubleIntegral(expr, xVar, yVar, ax, bx, ay, by, n = 16) {
  ...
  const nodes = _GL_NODES.slice(0, n),
    weights = _GL_WEIGHTS.slice(0, n);
  const midX = (bx + ax) / 2,
    halfX = (bx - ax) / 2;
  let total = 0;
  for (let i = 0; i < n; i++) {
